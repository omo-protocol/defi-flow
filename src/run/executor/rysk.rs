...
// ── Rysk Options Executor ──────────────────────────────────────────

pub struct RyskExecutor {
...
impl RyskExecutor {
    ...
    /// Select the best option matching criteria.
    fn select_option(
    ...
    async fn execute_sell(
        ...
        match selected {
            Some((key, entry)) => {
                ...
                // TODO: Submit actual RFQ order to Rysk contract on HyperEVM
    ...
    async fn execute_collect_premium(&mut self, asset: &RyskAsset) -> Result<ExecutionResult> {
        ...
        self.positions.retain(|pos| {
            if pos.asset == asset_key && pos.expiry <= now && pos.is_short {
                ...
                false // remove settled positions
            } else {
                true
            }
        });
    ...
    async fn execute_roll(
        ...
        for pos in &near_expiry {
            ...
            // Open new position with same collateral
            self.execute_sell(
                asset,
                pos.is_put,
                delta_target,
...
#[async_trait]
impl VenueExecutor for RyskExecutor {
