...
pub struct LendingExecutor {
    wallet_address: Address,
    private_key: String,
    dry_run: bool,
    /// Total supplied value (USD tracking)
    supplied_value: f64,
    /// Total borrowed value (USD tracking)
    borrowed_value: f64,
    metrics: SimMetrics,
}

impl LendingExecutor {
    ...
    /// Resolve the chain for a lending venue.
    fn venue_chain(venue: &LendingVenue) -> Chain {
        match venue {
            LendingVenue::HyperLend => Chain::HyperEvm,
            LendingVenue::Aave => Chain::Ethereum, // default, could also be Base/Arbitrum
    ...
    async fn execute_supply(
        ...
        let amount_units = evm::to_token_units(input_amount, 1.0, decimals); // assuming 1:1 for stables
    ...
    async fn execute_borrow(
        ...
        // interestRateMode = 2 for variable rate
        let pool = IAavePool::new(pool_addr, &provider);
    ...
    async fn execute_repay(
        ...
        // Repay (interestRateMode = 2 for variable)
        let pool = IAavePool::new(pool_addr, &provider);
...
#[async_trait]
impl VenueExecutor for LendingExecutor {
