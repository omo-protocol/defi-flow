...
// ── Aerodrome LP Executor ──────────────────────────────────────────

pub struct AerodromeExecutor {
    wallet_address: Address,
    private_key: String,
    dry_run: bool,
    /// Tracked NFT position token ID (set after mint).
    position_token_id: Option<U256>,
    /// Tracked gauge address for the position.
    gauge_address: Option<Address>,
    /// Total deposited value for TVL tracking.
    deposited_value: f64,
    metrics: SimMetrics,
}

impl AerodromeExecutor {
    pub fn new(config: &RuntimeConfig) -> Result<Self> {
        Ok(AerodromeExecutor {
    ...
    /// Parse pool pair "TOKEN0/TOKEN1" into token addresses on Base.
    ...
    async fn execute_add_liquidity(
        ...
        let lower = tick_lower.unwrap_or(-887220); // full range
        let upper = tick_upper.unwrap_or(887220);
        let position_manager = evm::aerodrome_position_manager();

        // Split input 50/50 between token0 and token1
        ...
        let params = INonfungiblePositionManager::MintParams {
            ...
            deadline,
            sqrtPriceX96: Uint::<160, 3>::ZERO, // 0 means use current price
        };
    ...
    async fn execute_remove_liquidity(&mut self, pool: &str) -> Result<ExecutionResult> {
        ...
        // TODO: call decreaseLiquidity + collect with actual tokenId
        let value = self.deposited_value;
        self.deposited_value = 0.0;
    ...
    async fn execute_claim_rewards(&mut self, pool: &str) -> Result<ExecutionResult> {
        ...
        if self.dry_run {
            ...
            // Estimate some reward amount based on deposited value
            let estimated_reward = self.deposited_value * 0.001; // ~0.1% per claim period
    ...
    async fn execute_compound(&mut self, pool: &str) -> Result<ExecutionResult> {
        ...
        // Compound = claim rewards + swap to pool tokens + add liquidity
        if self.dry_run {
...
#[async_trait]
impl VenueExecutor for AerodromeExecutor {
