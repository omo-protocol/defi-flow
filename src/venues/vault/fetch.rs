...
pub async fn fetch(
    ...
    if job.source != DataSource::DefiLlamaYield || job.kind != "vault" {
        return None;
    }
...
/// Fetch vault yield history from DefiLlama yields API.
async fn fetch_vault(
    client: &reqwest::Client,
    venue_slug: &str,
    asset: &str,
    config: &FetchConfig,
) -> Result<Vec<VaultCsvRow>> {
    ...
    let rows: Vec<VaultCsvRow> = chart
        .iter()
        .filter_map(|p| {
            ...
            Some(VaultCsvRow {
                timestamp: ts,
                apy: p.apy_base.unwrap_or(0.0) / 100.0,
