...
use crate::data::csv_types::OptionsCsvRow;
...
impl OptionsSimulator {
    ...
    fn current_snapshot(&self) -> Option<&OptionsSnapshot> {
        self.snapshots.get(self.cursor.min(self.snapshots.len().saturating_sub(1)))
    }

    fn advance_cursor(&mut self, clock: &SimClock) {
        let ts = clock.current_timestamp();
        while self.cursor + 1 < self.snapshots.len()
            && self.snapshots[self.cursor + 1].timestamp <= ts
        {
            self.cursor += 1;
        }
    }
    ...
    /// Select the best strike matching the node's delta_target and days_to_expiry.
    fn select_strike(
        &self,
        asset: &str,
        option_type: &OptionType,
        ...
        // Pick the strike closest to delta_target
...
impl VenueSimulator for OptionsSimulator {
    fn execute(
        &mut self,
        node: &Node,
        input_amount: f64,
        ...
        match action {
            ...
            OptionsAction::BuyCall | OptionsAction::BuyPut => {
                // Simplified: just consume the premium
                self.balance += input_amount;
                Ok(ExecutionResult::PositionUpdate {
                    consumed: input_amount,
                    output: None,
            ...
            OptionsAction::Roll => {
                // Close expiring + open new (settle handles close, execute handles open)
                Ok(ExecutionResult::Noop)
            }
            OptionsAction::Close => {
                let available = self.balance.max(0.0);
